      ...
      module InstanceMethods
        ...
        def related_search_options(context, klass, options = {})
          ...
          { :select     => "#{klass.table_name}.*, COUNT(#{Tag.table_name}.id) AS count", 
            ...
            :conditions => ["#{exclude_self} #{klass.table_name}.id = #{Tagging.table_name}.taggable_id AND #{Tagging.table_name}.taggable_type = '#{klass.class_name}' AND #{Tagging.table_name}.tag_id = #{Tag.table_name}.id AND #{Tag.table_name}.name IN (?)", tags_to_find],
